...
public class AthentoDocumentUpdateOperation {
	...
	public DocumentModel run(DocumentModel doc) throws Exception {
		if (_log.isDebugEnabled()) {
			_log.debug(AthentoDocumentUpdateOperation.ID + " BEGIN with params:");
		...
		String watchedDocumentType = String.valueOf(
			config.get(AthentoDocumentUpdateOperation.CONFIG_WATCHED_DOCTYPE));
		String operationId = String.valueOf(
			config.get(AthentoDocumentUpdateOperation.CONFIG_OPERATION_ID));
		...
		if (_log.isDebugEnabled()) {
			_log.debug(AthentoDocumentUpdateOperation.ID + " END return value: " + doc);
		}
		return doc;
	}
